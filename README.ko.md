...
**git 저장소**(GitHub, GitLab, Bitbucket, Gitea, GitHub Enterprise 등 smart-HTTP를 지원하는 모든 호스트), **문서 사이트 URL**(sitemap 우선 크롤러, robots.txt 준수), **파일 폴더**(`aws s3 sync`로 올리는 S3 업로드 prefix. PDF/Word/Excel은 빌드 시 Markdown으로 변환)를 등록합니다. 공개 범위는 public(공유, 허브 병합) 또는 private(격리) 중에서 고릅니다. **공개 서버 카탈로그**는 플랫폼의 모든 public 소스를 유형, 소유자, 빌드 상태, 구독자 수와 함께 보여 주며, 클릭 한 번으로 구독합니다. 소스 행마다 빌드 설정이 붙어 있습니다: 재빌드, 이름 변경, 멤버(private), 크롤 설정(문서 사이트), 파일 관리(파일 폴더), 그리고 Bedrock 모델·내장 이미지 추출·코퍼스 상한을 정하는 **AI 추출 설정**. 등록과 설정 변경, 멤버·파일 관리, 키 발급과 사용자 초대는 공통 모달에서 처리하고 목록은 조회에 집중합니다. 콘솔의 모든 표는 페이지로 나뉩니다.

**빌드 상세 / 로그**에서 최근 빌드의 단계, 실패 메시지와 CloudWatch 로그를 콘솔 안에서 확인합니다. 조치 안내를 보고 파일·AI 추출 설정을 수정한 뒤 재빌드할 수 있습니다. 로그는 필요할 때 조회하고 이전 로그를 추가로 불러옵니다. 로그 부재와 조회 권한 오류도 구분해 표시합니다. 자세한 내용은 [빌드 진단 안내](docs/build-diagnostics.md)를 참고하세요.

![소스](docs/screenshots/repos.ko.png)

### 소스 그룹: 코드와 프로젝트 문서 연결

프론트엔드·백엔드 저장소와 기획·QA 소스를 그룹으로 묶습니다. 소스별 역할과
설명을 작성하고 관계를 분석한 뒤, 양쪽 원문 근거를 확인하거나 그룹 MCP로
질의할 수 있습니다. 명시적인 참조는 모델 호출 없이 연결하며, 선택적인 Claude
분석은 추론 관계와 사용 토큰을 구분해 표시합니다. 그룹 조회는 필요할 때
Lambda에서 실행하므로 그룹마다 상시 Fargate 서버를 추가하지 않습니다.
[소스 그룹 안내](docs/source-groups.md)에 권한·버전·한도·비용 제어를 설명했습니다.
